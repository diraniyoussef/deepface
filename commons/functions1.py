...
def process_frames(cap, face_detector, embeddings_df, threshold, model, detector_backend = 'opencv', align = False, target_size = (224, 224), auto_add = False, emotion_model = None, normalization = "base", img_type = (".jpg", ".png")):
	...
	while(not (cv2.waitKey(1) & 0xFF == ord('q')) and ret == True):	
		...
		frame_info = process_frame(frame_index, img, face_detector, embeddings_df, threshold, model, detector_backend = detector_backend, align = align, target_size = (target_size[0], target_size[1]), process_then_show = True, auto_add = auto_add, emotion_model = emotion_model, normalization = normalization, img_type = img_type)
...
def process_frame(frame_index, img, face_detector, embeddings_df, threshold, model, detector_backend = 'opencv', align = False, target_size = (224, 224), process_then_show = True, auto_add = False, emotion_model = None, normalization = 'base', img_type = (".jpg", ".png")):
	face_detected = False
	...
	for face, (x, y, w, h) in faces:
		...
		if(not process_then_show): #show the rectangles and texts without returning them.
